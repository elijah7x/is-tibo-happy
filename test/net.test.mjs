...
const RAW = SOURCES.mirrors[0], JSD = SOURCES.mirrors[1], DIRECT = SOURCES.direct, BACKUP = SOURCES.backup;
...
const fake = map => (url) => {
  ...
  return Promise.resolve({ ok: v.ok ?? true, status: v.status ?? 200, json: async () => { if (v.bad) throw new SyntaxError('not json'); return v.body; } });
};
...
test('everything down → throws with all reasons', async () => {
  globalThis.fetch = fake({ [RAW]: new Error('a'), [JSD]: new Error('b'), [DIRECT]: new Error('c'), [BACKUP]: new Error('d') });
  await assert.rejects(fetchForecast('ua'), /all sources failed.*a.*b.*c.*d/s);
});
...
test('every request carries our UA', async () => {
  ...
  assert.deepEqual(seen, ['is-tibo-happy/test']);
});
