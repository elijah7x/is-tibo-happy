...
fn tease_default(quote: Value) -> Value {
    tease(quote, "2026-09-19T16:48:38.000Z", json!("2026-09-21T16:48:38.000Z"))
}
...
fn tease_tuesday_local_weekday_differs_by_timezone() {
    ...
    assert_eq!(run(&j, SH, NOW_MS), json!({"kind":"happy","zh":"预计周三重置","en":"reset expected Wednesday"}));
    assert_eq!(run(&j, LA, NOW_MS), json!({"kind":"happy","zh":"预计周二重置","en":"reset expected Tuesday"}));
    assert_eq!(run(&j, UTC, NOW_MS), json!({"kind":"happy","zh":"预计周二重置","en":"reset expected Tuesday"}));
    assert_eq!(run(&j, NZ, NOW_MS), json!({"kind":"happy","zh":"预计周三重置","en":"reset expected Wednesday"}));
}

#[test]
fn tease_tuesday_without_time_window_default_23utc() {
    let j = json!({"last_reset_at": LAST, "tease_signal": tease_default(json!("coming in Tuesday"))});
    assert_eq!(run(&j, SH, NOW_MS), json!({"kind":"happy","zh":"预计周三重置","en":"reset expected Wednesday"}));
}
...
fn tease_today_posted_this_morning() {
    ...
    assert_eq!(run(&j, UTC, NOW_MS), json!({"kind":"happy","zh":"预计今天重置","en":"reset expected today"}));
    assert_eq!(run(&j, SH, NOW_MS), json!({"kind":"happy","zh":"预计明天重置","en":"reset expected tomorrow"}));
}
...
fn tease_tomorrow_post_date_plus_1() {
    ...
    assert_eq!(run(&j, UTC, NOW_MS), json!({"kind":"happy","zh":"预计明天重置","en":"reset expected tomorrow"}));
    assert_eq!(run(&j, SH, NOW_MS), json!({"kind":"happy","zh":"预计周二重置","en":"reset expected Tuesday"}));
}
...
fn post_at_pt_monday_evening_tomorrow_is_pt_tuesday() {
    ...
    assert_eq!(run(&j, LA, now), json!({"kind":"happy","zh":"预计明天重置","en":"reset expected tomorrow"}));
    assert_eq!(run(&j, UTC, now), json!({"kind":"happy","zh":"预计今天重置","en":"reset expected today"}));
}
...
fn post_at_pt_tuesday_evening_tuesday_is_same_pt_tuesday() {
    ...
    assert_eq!(run(&j, LA, now), json!({"kind":"happy","zh":"即将重置","en":"reset imminent"}));
    assert_eq!(run(&j, SH, now), json!({"kind":"happy","zh":"即将重置","en":"reset imminent"}));
}
...
fn explicit_expires_at_beats_72h_default() {
    ...
    assert_eq!(run(&j, UTC, now), json!({"kind":"happy","zh":"有重置暗示","en":"reset hinted"}));
}
...
fn weekend_tonight_resolve_never_throw() {
    ...
    assert_eq!(run(&j("this weekend"), UTC, NOW_MS), json!({"kind":"happy","zh":"预计周六重置","en":"reset expected Saturday"}));
    assert_eq!(run(&j("tonight probably"), UTC, NOW_MS), json!({"kind":"happy","zh":"预计今天重置","en":"reset expected today"}));
}
...
fn invalid_tz_degrades_to_hinted() {
    ...
    assert_eq!(run(&j, "Mars/Olympus", NOW_MS), json!({"kind":"happy","zh":"有重置暗示","en":"reset hinted"}));
}
...
fn expired_tease_ignored_after_grace() {
    ...
    let j = json!({"last_reset_at": LAST, "tease_signal": tease_default(json!("coming in Tuesday")), "time_window": window()});
    assert_eq!(run(&j, UTC, now), json!({"kind":"unhappy","zh":"暂无重置预告","en":"no reset news"}));
}
...
fn tease_live_window_passed_imminent() {
    ...
    assert_eq!(run(&j, UTC, now), json!({"kind":"happy","zh":"即将重置","en":"reset imminent"}));
}

#[test]
fn tease_unparseable_quote_hinted() {
    let j = json!({"last_reset_at": LAST, "tease_signal": tease_default(json!("soon™"))});
    assert_eq!(run(&j, UTC, NOW_MS), json!({"kind":"happy","zh":"有重置暗示","en":"reset hinted"}));
}
...
fn dropped_tease_signal_but_latest_hint_fresh() {
    ...
    assert_eq!(run(&j, UTC, now), json!({"kind":"happy","zh":"预计今天重置","en":"reset expected today"}));
    assert_eq!(run(&j, SH, now), json!({"kind":"happy","zh":"预计明天重置","en":"reset expected tomorrow"}));
}
...
fn reset_landed_after_tease_fulfilled() {
    ...
    let now = ms("2026-09-23T01:00:00Z");
    assert_eq!(run(&j, UTC, now), json!({"kind":"happy","zh":"刚刚重置","en":"just reset"}));
}
...
fn resets_scheduled_for_overdue_within_grace_imminent() {
    ...
    assert_eq!(run(&j, UTC, NOW_MS), json!({"kind":"happy","zh":"即将重置","en":"reset imminent"}));
    ...
    assert_eq!(run(&done, UTC, NOW_MS), json!({"kind":"happy","zh":"刚刚重置","en":"just reset"}));
}

// ───────────── E. 明确承诺 → 倒计时 ─────────────
#[test]
fn commitment_countdown_granularity() {
    let c = |t: i64| json!({"last_reset_at": LAST, "commitment": {"scheduled_for": iso(NOW_MS + t)}});
    assert_eq!(run(&c((6.2 * D as f64) as i64), UTC, NOW_MS), json!({"kind":"happy","zh":"6 天后重置","en":"reset in 6 days"}));
    assert_eq!(run(&c((1.4 * D as f64) as i64), UTC, NOW_MS), json!({"kind":"happy","zh":"34 小时后重置","en":"reset in ~34h"}));
    assert_eq!(run(&c(3 * H), UTC, NOW_MS), json!({"kind":"happy","zh":"3 小时后重置","en":"reset in ~3h"}));
    assert_eq!(run(&c(30 * 60_000), UTC, NOW_MS), json!({"kind":"happy","zh":"即将重置","en":"reset imminent"}));
    assert_eq!(run(&c(D), UTC, NOW_MS), json!({"kind":"happy","zh":"24 小时后重置","en":"reset in ~24h"}));
    assert_eq!(run(&c(2 * D), UTC, NOW_MS), json!({"kind":"happy","zh":"2 天后重置","en":"reset in 2 days"}));
    assert_eq!(run(&c(D + D / 2), UTC, NOW_MS), json!({"kind":"happy","zh":"36 小时后重置","en":"reset in ~36h"}));
}

#[test]
fn commitment_just_passed_imminent_long_passed_still_imminent() {
    let c = |t: i64| json!({"last_reset_at": LAST, "commitment": {"scheduled_for": iso(NOW_MS + t)}});
    assert_eq!(run(&c(-60_000), UTC, NOW_MS), json!({"kind":"happy","zh":"即将重置","en":"reset imminent"}));
    assert_eq!(run(&c(-7 * H), UTC, NOW_MS), json!({"kind":"happy","zh":"即将重置","en":"reset imminent"}));
}

#[test]
fn commitment_overdue_beyond_grace_stale() {
    let j = json!({"last_reset_at": LAST, "commitment": {"scheduled_for": iso(NOW_MS - 2 * D)}});
    assert_eq!(run(&j, UTC, NOW_MS), json!({"kind":"unhappy","zh":"暂无重置预告","en":"no reset news"}));
}
...
fn commitment_beats_tease() {
    ...
    assert_eq!(run(&j, UTC, NOW_MS), json!({"kind":"happy","zh":"5 天后重置","en":"reset in 5 days"}));
}
...
fn teased_window_end_by_local_weekday() {
    ...
    assert_eq!(run(&j, LA, NOW_MS), json!({"kind":"happy","zh":"最晚周二重置","en":"reset by Tuesday"}));
    assert_eq!(run(&j, SH, NOW_MS), json!({"kind":"happy","zh":"最晚周三重置","en":"reset by Wednesday"}));
}
...
fn teased_window_alone_counts_overdue_imminent() {
    ...
    assert_eq!(run(&j, LA, late), json!({"kind":"happy","zh":"即将重置","en":"reset imminent"}));
...
fn ledger_recent_happy_over_3d_unhappy() {
    ...
    assert_eq!(run(&l(0.5), UTC, NOW_MS), json!({"kind":"happy","zh":"刚刚重置","en":"just reset"}));
    assert_eq!(run(&l(2.1), UTC, NOW_MS), json!({"kind":"happy","zh":"上次重置 2 天前","en":"last reset 2d ago"}));
    assert_eq!(run(&l(3.0), UTC, NOW_MS), json!({"kind":"happy","zh":"上次重置 3 天前","en":"last reset 3d ago"}));
    assert_eq!(run(&l(3.01), UTC, NOW_MS), json!({"kind":"unhappy","zh":"暂无重置预告","en":"no reset news"}));
    assert_eq!(run(&l(30.0), UTC, NOW_MS), json!({"kind":"unhappy","zh":"暂无重置预告","en":"no reset news"}));
}

#[test]
fn ledger_future_last_reset_just_reset() {
    assert_eq!(run(&json!({"last_reset_at": iso(NOW_MS + D)}), UTC, NOW_MS), json!({"kind":"happy","zh":"刚刚重置","en":"just reset"}));
}
...
fn garbage_upstream_never_throws() {
    ...
    assert_eq!(derive(&json!("<html>cloudflare</html>"), NOW_MS)["kind"], "offline");
    assert_eq!(derive(&json!([]), NOW_MS)["kind"], "offline");
    assert_eq!(derive(&json!(42), NOW_MS)["kind"], "offline");
    assert_eq!(run(&json!({}), UTC, NOW_MS), json!({"kind":"unhappy","zh":"暂无重置预告","en":"no reset news"}));
    assert_eq!(run(&json!({"last_reset_at": "garbage"}), UTC, NOW_MS), json!({"kind":"unhappy","zh":"暂无重置预告","en":"no reset news"}));
    assert_eq!(run(&json!({"age_days": "8"}), UTC, NOW_MS)["kind"], "unhappy");
    assert_eq!(run(&json!({"last_reset_at": LAST, "tease_signal": "not-an-object"}), UTC, NOW_MS)["kind"], "unhappy");
    assert_eq!(run(&json!({"last_reset_at": LAST, "tease_signal": {"post": null}}), UTC, NOW_MS)["kind"], "unhappy");
    assert_eq!(run(&json!({"last_reset_at": LAST, "commitment": "yes"}), UTC, NOW_MS)["kind"], "unhappy");
    assert_eq!(run(&json!({"last_reset_at": LAST, "time_window": {"start_hour": "x"}, "tease_signal": tease_default(json!("coming in Tuesday"))}), SH, NOW_MS)["zh"], "预计周三重置");
    assert_eq!(sub_line(&json!({"kind": "offline", "detail": {}}), NOW_MS, None)["zh"], "数据不可用");
}
...
fn resets_scheduled_for_exact_countdown() {
    ...
    assert_eq!(run(&j, UTC, NOW_MS), json!({"kind":"happy","zh":"4 天后重置","en":"reset in 4 days"}));
}

#[test]
fn resets_bare_string_scheduled() {
    assert_eq!(run(&json!({"scheduled": "2026-09-25T23:00:00.000Z", "events": [{"announced_at": LAST}]}), UTC, NOW_MS),
        json!({"kind":"happy","zh":"5 天后重置","en":"reset in 5 days"}));
    assert_eq!(run(&json!({"scheduled": "coming Tuesday", "events": [{"announced_at": LAST}]}), UTC, NOW_MS)["kind"], "unhappy");
    assert_eq!(run(&json!({"scheduled": "2026-09-15T23:00:00.000Z", "events": [{"announced_at": LAST}]}), UTC, NOW_MS)["kind"], "unhappy");
}
...
fn resets_ledger_rules_match_forecast() {
    assert_eq!(run(&json!({"events": [{"announced_at": iso(NOW_MS - D)}]}), UTC, NOW_MS), json!({"kind":"happy","zh":"上次重置 1 天前","en":"last reset 1d ago"}));
    // 落地后按 reset_type 分开描述：发卡型 vs 用量直充型；未知类型走通用文案
    assert_eq!(run(&json!({"events": [{"announced_at": iso(NOW_MS - D / 2), "reset_type": "banked"}]}), UTC, NOW_MS), json!({"kind":"happy","zh":"刚发了重置卡","en":"card just issued"}));
    assert_eq!(run(&json!({"events": [{"announced_at": iso(NOW_MS - 2 * D), "reset_type": "banked"}]}), UTC, NOW_MS), json!({"kind":"happy","zh":"上次发卡 2 天前","en":"card issued 2d ago"}));
    assert_eq!(run(&json!({"events": [{"announced_at": iso(NOW_MS - D / 2), "reset_type": "regular"}]}), UTC, NOW_MS), json!({"kind":"happy","zh":"用量刚重置","en":"usage just reset"}));
    assert_eq!(run(&json!({"events": [{"announced_at": iso(NOW_MS - D), "reset_type": "regular"}]}), UTC, NOW_MS), json!({"kind":"happy","zh":"用量 1 天前重置","en":"usage reset 1d ago"}));
    assert_eq!(run(&json!({"events": [{"announced_at": iso(NOW_MS - D), "reset_type": "weird"}]}), UTC, NOW_MS), json!({"kind":"happy","zh":"上次重置 1 天前","en":"last reset 1d ago"}));
    // 取最近一次事件的类型；更老的事件类型不冒名顶替
    assert_eq!(run(&json!({"events": [{"announced_at": iso(NOW_MS - 3 * D), "reset_type": "banked"}, {"announced_at": iso(NOW_MS - D / 2), "reset_type": "regular"}]}), UTC, NOW_MS), json!({"kind":"happy","zh":"用量刚重置","en":"usage just reset"}));
    assert_eq!(run(&json!({"events": [{"announced_at": LAST}]}), UTC, NOW_MS), json!({"kind":"unhappy","zh":"暂无重置预告","en":"no reset news"}));
    assert_eq!(run(&json!({"events": []}), UTC, NOW_MS), json!({"kind":"unhappy","zh":"暂无重置预告","en":"no reset news"}));
    assert_eq!(run(&json!({"events": [{"announced_at": "bad"}]}), UTC, NOW_MS), json!({"kind":"unhappy","zh":"暂无重置预告","en":"no reset news"}));
}

#[test]
fn derive_dispatches_on_shape() {
    assert_eq!(derive_forecast(&json!({"last_reset_at": LAST}), NOW_MS)["kind"], "unhappy");
    assert_eq!(derive_state(&json!({"events": [{"announced_at": LAST}]}), NOW_MS)["kind"], "unhappy");
    assert_eq!(derive(&json!({"events": []}), NOW_MS)["kind"], "unhappy");
    assert_eq!(derive(&json!({"last_reset_at": LAST}), NOW_MS)["kind"], "unhappy");
}
...
fn resolve_stale_or_absent_cache_offline() {
    ...
    assert_eq!(resolve_display(None, Some(&json!({"state": cached, "at": NOW_MS - 12 * H})), NOW_MS, None).0["kind"], "offline");
    assert_eq!(resolve_display(None, None, NOW_MS, None).0["kind"], "offline");
    assert_eq!(resolve_display(None, Some(&json!({"state": cached, "at": "garbage"})), NOW_MS, None).0["kind"], "offline");
    assert_eq!(resolve_display(None, Some(&json!({"state": "nope", "at": NOW_MS})), NOW_MS, None).0["kind"], "offline");
    let (o, _) = resolve_display(None, None, NOW_MS, None);
    assert_eq!(o["detail"]["sub"], json!({"zh": "数据不可用", "en": "data unavailable"}));
}

#[test]
fn resolve_cached_offline_never_kept() {
    let cache = json!({"state": {"kind": "offline", "detail": {}}, "at": NOW_MS});
    assert_eq!(resolve_display(None, Some(&cache), NOW_MS, None).0["kind"], "offline");
}

#[test]
fn resolve_bad_at_rejected() {
    let good = json!({"kind": "happy", "detail": {"sub": {"zh": "x", "en": "x"}}});
    assert_eq!(resolve_display(None, Some(&json!({"state": good, "at": (NOW_MS - H).to_string()})), NOW_MS, None).0["kind"], "offline");
    assert_eq!(resolve_display(None, Some(&json!({"state": good, "at": f64::NAN})), NOW_MS, None).0["kind"], "offline");
    assert_eq!(resolve_display(None, Some(&json!({"state": good})), NOW_MS, None).0["kind"], "offline");
    assert_eq!(resolve_display(None, Some(&json!({"state": good, "at": [1, 2]})), NOW_MS, None).0["kind"], "offline");
}

#[test]
fn resolve_future_at_no_immortality() {
    let good = json!({"kind": "happy", "detail": {"sub": {"zh": "x", "en": "x"}}});
    assert_eq!(resolve_display(None, Some(&json!({"state": good, "at": NOW_MS + 30 * 60_000})), NOW_MS, None).0["kind"], "happy");
    assert_eq!(resolve_display(None, Some(&json!({"state": good, "at": NOW_MS + 72 * H})), NOW_MS, None).0["kind"], "offline");
}
...
fn resolve_old_kind_waiting_not_kept() {
    ...
    assert_eq!(resolve_display(None, Some(&waiting), NOW_MS, None).0["kind"], "offline");
    assert_eq!(resolve_display(None, Some(&nodetail), NOW_MS, None).0["kind"], "offline");
}
...
fn resolve_offline_fresh_does_not_overwrite() {
    ...
    let (s, c) = resolve_display(Some(&json!({"kind": "offline", "detail": {}})), Some(&cache), NOW_MS, None);
...
fn resolve_cached_state_reworded_for_now() {
    ...
    let mut s = derive(&json!({"last_reset_at": LAST, "tease_signal": tease(json!("coming today"), "2026-09-21T10:00:00Z", json!("2026-09-23T00:00:00Z")), "time_window": window()}), written);
    ...
    let mut l = derive(&json!({"last_reset_at": iso(NOW_MS - (2.5 * D as f64) as i64)}), NOW_MS);
    l["detail"]["sub"] = sub_line(&l, NOW_MS, None);
    let cache = json!({"state": l, "at": NOW_MS});
    assert_eq!(resolve_display(None, Some(&cache), NOW_MS + 10 * H, None).0["detail"]["sub"]["zh"], "上次重置 2 天前");
}
...
impl Default for Bp {
    fn default() -> Self {
        Bp { pid: "codex", today: "none", last: "2026-09-12T08:09:17.000Z", card: true, status: "scheduled", sf: "2026-09-22T07:00:00.000Z", pa: "2026-09-19T16:48:38.000Z" }
    }
}
fn bp(o: Bp) -> String {
    format!(
...
fn betteropc_parses_dom_and_rsc() {
    ...
    assert_eq!(p["commitment"]["url"], "https://x.com/thsottiaux/status/2101352781219258527");
}

#[test]
fn betteropc_0700z_is_today_for_mainland_china() {
    let now = ms("2026-09-22T04:00:00Z");
    assert_eq!(run_bp(Bp::default(), SH, now), json!({"kind":"happy","zh":"3 小时后重置","en":"reset in ~3h"}));
    assert_eq!(run_bp(Bp::default(), SH, ms("2026-09-22T08:00:00Z")), json!({"kind":"happy","zh":"即将重置","en":"reset imminent"}));
    assert_eq!(run_bp(Bp::default(), SH, ms("2026-09-22T20:00:00Z")), json!({"kind":"happy","zh":"即将重置","en":"reset imminent"}));
}

#[test]
fn betteropc_scheduled_for_stale_falls_to_ledger() {
    let now = ms("2026-09-24T12:00:00Z");
    assert_eq!(run_bp(Bp::default(), UTC, now), json!({"kind":"unhappy","zh":"暂无重置预告","en":"no reset news"}));
}

#[test]
fn betteropc_reset_landed_fulfilled() {
    let now = ms("2026-09-22T08:00:00Z");
    let s = derive(&parse_betteropc(&bp(Bp { last: "2026-09-22T07:05:00.000Z", ..Default::default() })).unwrap(), now);
    assert_eq!(s["kind"], "happy");
    assert_eq!(sub_line(&s, now, Some(SH)), json!({"zh": "刚刚重置", "en": "just reset"}));
}

#[test]
fn betteropc_terminal_status_not_signal() {
    for status in ["executed", "cancelled", "missed", "done", "已取消", "已执行", "已重置"] {
        let now = ms("2026-09-22T04:00:00Z");
        assert_eq!(run_bp(Bp { status, ..Default::default() }, UTC, now)["kind"], "unhappy", "status={status}");
    }
}
...
fn betteropc_no_card_ledger_only() {
    let now = ms("2026-09-22T04:00:00Z");
    assert_eq!(run_bp(Bp { card: false, ..Default::default() }, UTC, now)["kind"], "unhappy");
    assert_eq!(run_bp(Bp { card: false, last: "2026-09-22T02:00:00.000Z", ..Default::default() }, UTC, now),
        json!({"kind":"happy","zh":"刚刚重置","en":"just reset"}));
    assert_eq!(run_bp(Bp { sf: "not-a-date", ..Default::default() }, UTC, now),
        json!({"kind":"happy","zh":"已预告重置","en":"reset announced"}));
    assert_eq!(run_bp(Bp { sf: "not-a-date", pa: "", ..Default::default() }, UTC, now)["kind"], "unhappy");
    assert_eq!(run_bp(Bp { pa: "", ..Default::default() }, UTC, now)["kind"], "happy");
}
...
fn betteropc_garbage_null_never_throws() {
    ...
    assert!(parse_betteropc("{}").is_none());
    assert!(parse_betteropc(&bp(Bp { pid: "claude-code", ..Default::default() })).is_none());
    // 正文提到 Codex 也不顶用：守卫按身份不认字样
    let tampered = bp(Bp::default()).replacen("data-product-id", "data-x-product-id", 1) + "<p>Codex 与 Claude Code 对比</p>";
    ...
    let bad = parse_betteropc(&bp(Bp { pa: "", last: "garbage", ..Default::default() })).unwrap();
...
fn betteropc_props_order_drift_no_neighbour_steal() {
    ...
    assert_eq!(parse_betteropc(reordered).unwrap()["last_reset_at"], "2026-09-12T08:09:17.000Z");
}
